...
# Định nghĩa các cấu hình cố định cho Agent
...
async def run_invoice_extraction(invoice_text: str) -> Any:
    ...
    LƯU Ý QUAN TRỌNG VỀ CONCURRENCY: Để hỗ trợ xử lý ĐỒNG THỜI, chúng ta tạo một instance Agent 
    mới cho mỗi request. Strands Agent hiện tại không hỗ trợ gọi đồng thời trên cùng một instance.
    ...
    """
    # 1. Kiểm tra sơ bộ input text
    clean_text = invoice_text.strip()
    if not clean_text:
        raise ValueError("Văn bản hóa đơn trống. Vui lòng cung cấp nội dung hóa đơn.")
    
    if len(clean_text) < 20: # Ví dụ: "Hóa đơn 345" là quá ngắn
        raise ValueError(
            f"Nội dung hóa đơn quá ngắn ({len(clean_text)} ký tự). "
            "Vui lòng cung cấp đầy đủ thông tin hóa đơn (bao gồm người bán, người mua, danh sách sản phẩm, v.v.) để AI có thể trích xuất chính xác."
        )

    # 2. Khởi tạo Agent mới cho mỗi request để tránh lỗi Concurrency
    invoice_agent = Agent(
        model=ollama_model_provider,
        system_prompt=INVOICE_SYSTEM_PROMPT,
        tools=[clean_invoice_text],
    )
...
- Tổng tiền và các thông tin tính toán khác"""
    ...
    try:
        ...
        raise ValueError(
            "Dữ liệu đầu vào không đủ thông tin để trích xuất hóa đơn. "
            "Vui lòng cung cấp văn bản hóa đơn đầy đủ hơn (bao gồm: Tên công ty, Danh sách sản phẩm, Tổng tiền, v.v.)."
        )
    
    except Exception as e:
        # Kiểm tra nếu là lỗi concurrency (dù đã tạo agent mới nhưng vẫn check cho chắc)
        if "Concurrent invocations are not supported" in str(e):
             raise RuntimeError("Hệ thống đang bận xử lý một request khác. Vui lòng thử lại sau giây lát.")
             
        raise RuntimeError(
            f"Lỗi hệ thống khi trích xuất hóa đơn: {str(e)}. "
            f"Vui lòng thử lại sau giây lát."
        )
